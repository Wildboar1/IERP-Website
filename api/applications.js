...
export default async function handler(req, res) {
  ...
  // Extract ID and action from URL path
  const pathParts = req.url.split('/').filter(p => p);
  const idIndex = pathParts.indexOf('applications') + 1;
  const id = idIndex > 0 && idIndex < pathParts.length ? pathParts[idIndex] : null;
  const action = idIndex > 0 && idIndex + 1 < pathParts.length ? pathParts[idIndex + 1] : null;

  // Only handle PATCH requests for /{id}/status
